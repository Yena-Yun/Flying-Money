...
* React를 만든 페이스북에서 만들어서(= 같은 곳에서 개발하여) React의 스타일에 맞는 라이브러리
* atom이라는 단위로 개별적으로 관리하기 때문에 전역 상태의 추적 및 유지 보수가 용이
* Redux를 쓸 만큼 프로젝트 규모가 크지 않다고 판단, 무거운 boilerplate 없이 비교적 빠르게 사용 가능한 라이브러리 선택
