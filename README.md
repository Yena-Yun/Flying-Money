# WYE-WYA (What You Expend is What You Are)

수기로 쓰고 있는 가계부의 점점 날아가는 글씨(?)에 지쳐 <br/>
웹 서비스로 한눈에 파악하려고 만들어보는 토이 프로젝트
...
// 지출 내역
type Transaction = {
  id: number(string?), // 랜덤 함수? uuid4? 고민중
  date: string?, // 달력 라이브러리 사용할 예정
  diary?: string[], // 그날 있었던 일 간단 기록 (문자열 배열, optional)
  place: string, // 지출한 장소 (title과 비슷한 기능)
  items: Item[], // 각 장소에서 지출한 상품과 서비스 항목
}
